...
func parseFlags(args []string) (config, error) {
	...
	fs.StringVar(&cfg.Admin, "admin", cfg.Admin, "proxy admin (required for proxy deploys; must differ from owner)")
...
func isProxiedContract(name string) bool {
	switch strings.ToLower(strings.TrimSpace(name)) {
	case "erc1967factory", "factory", "decimalquoter", "swaprouter", "rescuevault", "rescue":
		return false
	default:
		return true
	}
...
// resolveAdmin returns the proxy admin. For proxied contracts --admin is
// required and must differ from the owner, otherwise seal is not a commitment.
...
func runOne(ctx context.Context, d *publish.Deployer, cfg config, owner, admin, feeAddress, protocolRecipient, periodPoker, baseCurrency common.Address) error {
	...
	case "rescuevault", "rescue":
		addr, err := deployPlain(ctx, d, "RescueVault", rescuevault.InitCode(rescuevault.ConstructorArgs{Admin: admin}), rescuevault.GasLimit)
		if err != nil {
			return err
		}
		out.Implementations["rescuevault"] = addr.Hex()
...
func deployProxied(ctx context.Context, d *publish.Deployer, factory, admin common.Address, name string, bytecode []byte, implGasLimit uint64, initFn func() ([]byte, error)) (common.Address, common.Address, error) {
	...
	txHash, err := d.DeployProxy(ctx, factory, implAddr, admin, salt, initData, publish.ProxyGasLimit)
	if err != nil {
...
func contractBytecode(contract string, admin common.Address) ([]byte, uint64, error) {
	...
	case "rescuevault", "rescue":
		return rescuevault.InitCode(rescuevault.ConstructorArgs{Admin: admin}), rescuevault.GasLimit, nil
	case "splitter":
...
func runDeployImpl(cfg config) error {
	...
	admin := deployerAddr
	if cfg.Owner != "" {
		admin, err = parseAddress(cfg.Owner)
		if err != nil {
			return err
		}
	}
	...
	bytecode, gasLimit, err := contractBytecode(cfg.Contract, admin)
	if err != nil {
		return err
	}
	...
	outKey := strings.ToLower(strings.TrimSpace(cfg.Contract))
	if outKey == "rescue" {
		outKey = "rescuevault"
	}
	out := report{Implementations: map[string]string{outKey: addr.Hex()}}
	blob, err := json.MarshalIndent(out, "", "  ")
	if err != nil {
		return err
	}
...
func runDeployProxy(cfg config) error {
	...
	salt := publish.GenerateSalt(d.Address(), cfg.Contract)
	txHash, err := d.DeployProxy(ctx, factoryAddr, implAddr, admin, salt, initData, publish.ProxyGasLimit)
	if err != nil {
